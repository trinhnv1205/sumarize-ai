const appscriptId = 'AKfycbyixiClEWYV72m16StPWNLz8wqJCo0vhRC3V1lCFIIlwIr5BtCRjBYe1hatcUjPlU2_';

function saveSelectedLanguage(language) {
    console.log('saveSelectedLanguage:', language);
    browser.runtime.sendMessage({
        command: 'saveSettingLanguage', language: language
    });
}
...
function saveSelectedLength(length) {
    ...
    browser.runtime.sendMessage({
        command: 'saveSettingLength', length: length
    });
}
...
function summarizeText() {
    ...
    fetchWithRetry(endpoint, options)
        ...
        .catch(error => {
            ...
            document.getElementById('popup-content').textContent = 'Error fetching data';
            document.getElementById('loading-spinner').style.display = 'none'; // Hide spinner
        });
}
...
function translateText(text) {
    ...
    let targetLanguage = document.getElementById('language-select').value;  // Default to Vietnamese
    let targetLength = document.getElementById('length-select').value;  // Default to Medium

    let endpoint = `https://script.google.com/macros/s/${appscriptId}/exec?path=translate`
    fetch(endpoint, {
        method: 'POST', headers: {
            'Content-Type': 'application/json'
        }, body: JSON.stringify({
            text: text, target_language: targetLanguage, target_length: targetLength  // Include the selected length
        })
    })
        .then(response => response.json())
        .then(data => {
            // data will be like 
            // <h1>Summarize</h1><ul><li>Summarize text on any website</li><li>Translate text to any language</li><li>Copy text to clipboard</li></ul>

            // Parse the sanitized HTML string into DOM nodes
            const parser = new DOMParser();
            const doc = parser.parseFromString(data.translation, 'text/html');
        ...
        .catch(error => {
            console.error('Error:', error);
            document.getElementById('popup-content').textContent = 'Error translating text';
            document.getElementById('loading-spinner').style.display = 'none'; // Hide spinner
        });
}
...
function createLengthSelect(popupContainer) {
    ...
    const lengths = [{ code: 'short', name: 'Short' }, { code: 'medium', name: 'Medium' }, { code: 'long', name: 'Long' },];
...
function makeDraggable(element) {
    ...
    function onMouseMove(e) {
        if (isDragging) {
            const newX = Math.max(0, Math.min(e.clientX - offsetX, screenWidth - element.offsetWidth));
            const newY = Math.max(0, Math.min(e.clientY - offsetY, screenHeight - element.offsetHeight));
...
(function initializePopup() {
    ...
    createCopyButton(popupContainer);
    createTranslateButton(popupContainer);
